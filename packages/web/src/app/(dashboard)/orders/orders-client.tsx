...
import { getOrders, getOrderItems, type Order, type OrderItem } from "./actions";
...
interface OrdersClientProps {
  initialOrders: Order[];
  initialStats: { totalOrders: number; totalRevenue: number; withCoupon: number; withoutCoupon: number };
}
...
export function OrdersClient({ initialOrders, initialStats }: OrdersClientProps) {
  ...
  const loadData = useCallback(async (source?: string, coupon?: string) => {
    ...
    const ordersRes = await getOrders(filters);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        <select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)} className="flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm">
          <option value="">Todas as fontes</option>
          <option value="yampi">Yampi</option>
          <option value="shopify">Shopify</option>
        </select>
        <select value={couponFilter} onChange={(e) => setCouponFilter(e.target.value)} className="flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm">
